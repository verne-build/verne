...
type ProgressCallback = (state: SpeechModelState) => void
...
export class ModelManager {
  ...
  async getModelState(modelId: string): Promise<SpeechModelState> {
    ...
    if (cached && (
      cached.status === 'downloading' || cached.status === 'extracting' || cached.status === 'error'
    )) {
      return cached
    }
  ...
  async downloadModel(modelId: string): Promise<void> {
    ...
    try {
      ...
      this.updateState(modelId, 'extracting', 0.9)
      ...
      this.updateState(modelId, 'extracting', 0.95)
      ...
      this.updateState(modelId, 'extracting', 0.98)
  ...
  private updateState(
    ...
    const stateProgress = progress ?? (
      status === 'extracting' ? 0.95 : status === 'ready' ? 1 : undefined
    )
    const state: SpeechModelState = { id: modelId, status, progress: stateProgress, error }
    this.modelStates.set(modelId, state)

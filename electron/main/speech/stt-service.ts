...
export class SttService {
  ...
  private async _startDictation(
    ...
    if (
      ...
      this.eventSink = sink
      // Why: the warm worker may still hold buffered audio if the prior session
      // ended abnormally (error, never stopped). Clear it so this recording
      // can't decode concatenated with the previous one.
  ...
  private getWorkerPath(): string {
    // Verne's electron-vite build emits main-process bundles as .cjs.
    if (app.isPackaged) {
      return join(process.resourcesPath, 'app.asar', 'out', 'main', 'stt-worker.cjs')
    }
    return join(__dirname, 'stt-worker.cjs')
  }
  ...
  private scheduleIdleTeardown(): void {
    ...
    // the ONNX model after a quiet period so long-running sessions don't pin
    // speech memory forever.

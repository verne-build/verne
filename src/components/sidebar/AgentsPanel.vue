...
<template>
  <div class="relative flex h-full min-h-0 flex-col">
    ...
    <div class="agent-list flex flex-col gap-0.5">
    <SortableItem
      v-for="i in items"
      :id="i.tab.id"
    ...
    >
      <div class="flex h-4 w-full min-w-0 items-center gap-1.5 leading-none">
        ...
        <span class="min-w-0 truncate text-left text-foreground">{{ agentTitle(i.tab.id) }}</span>
        <img
      ...
      <div class="flex h-3.5 w-full min-w-0 items-center gap-1.5 pl-[1.125rem] text-[10px] leading-none">
        ...
        />
        <span class="ml-auto min-w-0 truncate text-right text-muted-foreground">{{ i.directory.name }}</span>
      </div>
    </Button>
      </ContextMenuTrigger>

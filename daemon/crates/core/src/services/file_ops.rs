...
fn codex(tool_name: &str, tool_input: &serde_json::Value) -> Vec<FileOperation> {
    ...
    let patch_text = tool_input
        ...
        .and_then(|v| v.as_str());
    let Some(text) = patch_text else { return vec![]; };

    let mut ops = Vec::new();
    for line in text.lines() {
        let trimmed = line.trim();
        if let Some(path) = trimmed.strip_prefix("*** Update File: ") {
            ops.push(FileOperation { op: FileOpType::Edit, file_path: path.trim().to_string() });
        } else if let Some(path) = trimmed.strip_prefix("*** Add File: ") {
            ops.push(FileOperation { op: FileOpType::Write, file_path: path.trim().to_string() });
...
mod tests {
    ...
    fn claude_read_write_edit_via_file_path() {
        let ops = extract_file_ops("claude", "Read", &serde_json::json!({ "file_path": "/abs/path/x.rs" }));
        assert_eq!(ops.len(), 1);
        assert_eq!(ops[0].op, FileOpType::Read);
        assert_eq!(ops[0].file_path, "/abs/path/x.rs");
        assert_eq!(extract_file_ops("claude", "Edit", &serde_json::json!({ "file_path": "/y.rs" }))[0].op, FileOpType::Edit);
        assert_eq!(extract_file_ops("claude", "Write", &serde_json::json!({ "file_path": "/z.rs" }))[0].op, FileOpType::Write);
    }

    #[test]
    fn claude_ignores_bash_and_missing_path() {
        assert!(extract_file_ops("claude", "Bash", &serde_json::json!({ "command": "ls" })).is_empty());
    ...
    fn unknown_agent_uses_claude_default() {
        ...
        let ops = extract_file_ops("opencode", "Read", &serde_json::json!({ "file_path": "/a" }));
        assert_eq!(ops.len(), 1);
        assert_eq!(ops[0].op, FileOpType::Read);
    }
    ...
    fn copilot_read_via_file_path_write_via_path_edit() {
        assert_eq!(extract_file_ops("copilot", "read", &serde_json::json!({ "file_path": "/abs/x" }))[0].op, FileOpType::Read);
        ...
        assert_eq!(extract_file_ops("copilot", "edit", &serde_json::json!({ "file_path": "/abs/e" }))[0].op, FileOpType::Edit);
    }

    #[test]
    fn copilot_ignores_unknown_tool() {
        assert!(extract_file_ops("copilot", "foo", &serde_json::json!({ "file_path": "/abs/z" })).is_empty());
    }

    // --- codex ---
    #[test]
    fn codex_parses_update() {
        let ops = extract_file_ops("codex", "apply_patch", &serde_json::json!({
    ...
    fn codex_multi_file_and_command_fallback_and_empty_and_non_apply_patch() {
        let multi = extract_file_ops("codex", "apply_patch", &serde_json::json!({
            "input": "*** Begin Patch\n*** Update File: a.rs\n@@\n*** Update File: b.rs\n@@\n*** End Patch"
        }));
        assert_eq!(multi.len(), 2);
        assert_eq!(multi[0].file_path, "a.rs");
        assert_eq!(multi[1].file_path, "b.rs");
        assert_eq!(extract_file_ops("codex", "apply_patch", &serde_json::json!({
            "command": "*** Begin Patch\n*** Update File: x.rs\n*** End Patch"
        })).len(), 1);
        assert!(extract_file_ops("codex", "apply_patch", &serde_json::json!({})).is_empty());
        assert!(extract_file_ops("codex", "shell_command", &serde_json::json!({ "command": "ls" })).is_empty());
    }

    #[test]
    fn codex_ignores_non_apply_patch_tool_even_with_markers() {
        assert!(extract_file_ops("codex", "shell_execute", &serde_json::json!({
